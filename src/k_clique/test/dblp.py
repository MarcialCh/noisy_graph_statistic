...
import os.path
from k_clique import two_phase, improve, baseline
from src.util.utils import induced_subgraph, k_cliques
import time
...
def load_edges():
    global network
    edge_file = open("D:/file/noisy_graph_statistic/src/dblp.txt", "r")
...
if __name__ == '__main__':
    ...
    for variance in range(1, 6):
        #total_price = baseline.total_price_from_variance(network, 7 * variance, price_points, k, 10**2.5)
        total_price = improve.total_price_from_variance(network, 7 * variance, price_points, k,10 ** 2.5, 1000)
        #total_price = two_phase.total_price_from_variance(network ,7*variance,price_points,k,10**2.5,1000)
        print("%s:variance %f" % (variance, total_price / n))


    # Exp-3: micro-payment for data sellers.
    for variance in range(1, 6):
        min_price, mean_price_1, mean_price_2, max_price = improve.min_max_price_from_variance(network, 7 * variance,price_points, k, 10 ** 2.5, 1000)
        #min_price, mean_price_1, mean_price_2, max_price = two_phase.min_max_price_from_variance(network,  7 * variance, price_points, k, 10**2.5,1000)
    ...
    for data_proportion in range(1, 11):
        ...
        #total_price = baseline.total_price_from_variance(subgraph, 7, price_points, k, 10**3)
        total_price = improve.total_price_from_variance(subgraph, 7, price_points, k, 10 ** 2, 1000)
        #total_price = two_phase.total_price_from_variance(subgraph, 7, price_points, k, 1)
    ...
    for k_examine in range(3, 11):
        ...
        subgraph = induced_subgraph(network, subgraph_node_num)
        time_start = time.perf_counter()
        #total_price = baseline.total_price_from_variance(subgraph, 7, price_points, k_examine, 10**3)
        total_price = improve.total_price_from_variance(subgraph, 7, price_points, k_examine, 10 ** 2, 1000)
        #total_price = two_phase.total_price_from_variance(subgraph, 7, price_points, k_examine, 1)
        time_end = time.perf_counter()
