...
import os.path
from k_star import upper, baseline
from src.util.utils import induced_subgraph, C
import time
...
if __name__ == '__main__':
    ...
    for variance in range(1, 6):
        ...
        #total_price = upper.total_price_from_variance(network, 7 * variance, price_points, k, 10 ** 2.5, 1000)
    ...
    for data_proportion in range(1, 11):
        ...
        #total_price = upper.total_price_from_variance(subgraph, 7, price_points, k, 1)
        time_end = time.perf_counter()
        print("%s:data_proportion %f" % (data_proportion, time_end - time_start))

    for k_examine in range(2, 11):
        subgraph_node_num = n
        subgraph = indced_subgraph(network, subgraph_node_num)
        time_start = time.perf_counter()
        total_price = baseline.total_price_from_variance(subgraph, 7, price_points, k_examine, 10**3)
        #total_price = upper.total_price_from_variance(subgraph, 7, price_points, k_examine, 10 ** 2, 1000)
        time_end = time.perf_counter()
